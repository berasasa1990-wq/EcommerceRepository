document.addEventListener('DOMContentLoaded', () => {
        ...
        thumb.addEventListener('click', () => {
            ...
            if (src) {
                ...
                if (thumb.dataset.full) {
                    mainImage.dataset.fullSrc = thumb.dataset.full;
                }
    ...
    (function initProductImageZoom() {
        const wrap = document.getElementById('mainProductImageWrap');
        const overlay = document.getElementById('productImageZoom');
        const stage = document.getElementById('productImageZoomStage');
        const zoomImg = document.getElementById('productImageZoomImg');
        const closeBtn = document.getElementById('productImageZoomClose');
        if (!wrap || !overlay || !stage || !zoomImg || !mainImage || mainImage.tagName !== 'IMG') return;
        ...
        function zoomSrc() {
            return mainImage.dataset.fullSrc || mainImage.currentSrc || mainImage.src;
        }
        ...
        function openZoom() {
            ...
            document.body.classList.add('pd-zoom-open');
            resetTransform();
            closeBtn?.focus();
        }
        ...
        function closeZoom() {
            overlay.hidden = true;
            document.body.classList.remove('pd-zoom-open');
            resetTransform();
            pointers.clear();
            pinch = null;
        }
        ...
        document.addEventListener('keydown', (event) => {
            if (event.key === 'Escape' && !overlay.hidden) closeZoom();
        });

        stage.addEventListener('wheel', (event) => {
            if (overlay.hidden) return;
            event.preventDefault();
            const next = scale + (event.deltaY < 0 ? 0.18 : -0.18);
        ...
        stage.addEventListener('pointermove', (event) => {
            ...
            pointers.set(event.pointerId, { x: event.clientX, y: event.clientY });
            if (pointers.size === 2 && pinch && pinch.dist) {
                moved = true;
                const pts = Array.from(pointers.values());
                const dist = Math.hypot(pts[0].x - pts[1].x, pts[0].y - pts[1].y);
                scale = Math.min(4, Math.max(1, pinch.scale * (dist / pinch.dist)));
                if (scale === 1) {
                    tx = 0;
                    ty = 0;
                }
            ...
            if (Math.abs(event.clientX - prev.x) > 6 || Math.abs(event.clientY - prev.y) > 6) {
                moved = true;
            }
    ...
    const gratisHeadline = document.getElementById('gratisOfferHeadline');
    const gratisLabel = document.getElementById('gratisOfferLabel');
    ...
    const gratisQtyInput = document.getElementById('gratisOfferQty');
    const gratisQtyMinus = document.getElementById('gratisOfferQtyMinus');
    const gratisQtyPlus = document.getElementById('gratisOfferQtyPlus');
    ...
    function setGratisOfferQty(value) {
        if (!gratisQtyInput) return;
        let n = parseInt(value, 10);
        if (!Number.isFinite(n) || n < 1) n = 1;
        if (n > 99) n = 99;
        gratisQtyInput.value = String(n);
        if (gratisQtyMinus) gratisQtyMinus.disabled = n <= 1;
        if (gratisQtyPlus) gratisQtyPlus.disabled = n >= 99;
    }

    function syncGratisQtyButtons() {
        setGratisOfferQty(getGratisOfferQty());
    }
    ...
    function fillGratisOfferModal(offer) {
        if (gratisLabel) {
            gratisLabel.textContent = offer.label || 'Dobra kupovina';
        }
        if (gratisTitle) gratisTitle.textContent = offer.gratis_naziv || '';
        if (gratisHeadline) {
            gratisHeadline.textContent = offer.headline || '';
            gratisHeadline.hidden = !offer.headline;
        }
        if (gratisText) {
        ...
        if (gratisImage) {
            if (offer.slika_url) {
                ...
                gratisImage.removeAttribute('src');
                gratisImage.alt = '';
                gratisImage.hidden = true;
            }
        }
        ...
        if (gratisBadge) {
            if (offer.badge) {
                gratisBadge.textContent = offer.badge;
                gratisBadge.hidden = false;
            } else {
                gratisBadge.textContent = '';
                gratisBadge.hidden = true;
            }
        }
        setGratisOfferQty(1);
