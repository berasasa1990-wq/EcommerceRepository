(function ($) {
    ...
    function flashTriggerVal() {
        return ($('#id_flash_trigger').val() || 'offer_product').toString();
    }
    ...
    function isFlashInline($el) {
        if ($el.hasClass('akcija-inline-flash-lines') || $el.find('.akcija-inline-flash-lines').length) {
            return true;
        }
        var s = (
            ($el.attr('id') || '') +
        ...
        return s.indexOf('akcijska') !== -1 || s.indexOf('flash') !== -1;
    }
    ...
    function toggleAkcijaFields() {
        ...
        var isAkcijska = tip === AKCIJSKA_TIP;
        ...
        allFieldsets().each(function () {
            var $fs = $(this);
            // Inline tabele i datetime widget (pocetak) imaju ugniježđeni fieldset —
            // ne smiju se sakriti ovdje, inače nestane unos artikala / datuma.
            if ($fs.closest('.inline-group, .js-inline-admin-formset, .tabular').length) {
                return;
            }
            if ($fs.closest('.form-row').length) {
                return;
            }
        ...
        $inlines.each(function () {
            ...
            if (isBundle && isBundleInline($g)) {
                show = true;
            } else if (isAkcijska && isFlashInline($g)) {
                show = true;
            ...
            if (isPonuda || isQty) {
                if (isDwellInline($g) || isBundleInline($g) || isFlashInline($g)) {
                    show = false;
                }
            }
            if (isBundle && (isDwellInline($g) || isFlashInline($g))) {
                show = false;
            }
            if (isAkcijska && (isBundleInline($g) || isDwellInline($g) || isQtyInline($g))) {
                show = false;
            }
            if (isAi && (isBundleInline($g) || isQtyInline($g) || isFlashInline($g))) {
                show = false;
            }
            setHidden($g, !show);
            // Bundle / akcijska ponuda: forsira prikaz tabele i autocomplete polja
            if (show && ((isBundle && isBundleInline($g)) || (isAkcijska && isFlashInline($g)))) {
                $g.find('fieldset, .module, .tabular, .form-row, tr.form-row, tr, td, .related-widget-wrapper').each(function () {
                    var $el = $(this);
                    $el.removeClass('akcija-section-hidden').removeAttr('hidden');
                    if ($el.is('tr, td, .form-row, .related-widget-wrapper, fieldset, .module, .tabular')) {
                        $el.css('display', '');
                    }
                });
        ...
        // 6a) Akcijska ponuda: forsira prikaz tabele artikala (Django 6 fieldset.module)
        if (isAkcijska) {
            var $flash = formRoot().find(
                '#flash_lines-group, .akcija-inline-flash-lines, ' +
                '.inline-group[id*="flash"]'
            );
            if (!$flash.length) {
                $flash = $('#flash_lines-group, .akcija-inline-flash-lines');
            }
            $flash.each(function () {
                var $g = $(this).hasClass('inline-group')
                    ? $(this)
                    : ($(this).closest('.inline-group').length ? $(this).closest('.inline-group').addBack() : $(this));
                setHidden($g, false);
                $g.find('fieldset, .module, .tabular, table, tr, td, .form-row').each(function () {
    ...
    function bind() {
        ...
        $(document)
            .off('change.akcijaTip', '#id_flash_trigger')
            .on('change.akcijaTip', '#id_flash_trigger', toggleAkcijaFields);
