...
class MagacinViewTests(TestCase):
    ...
    def test_artikal_etiketa_print(self):
        ...
        self.assertContains(page, 'column-gap: 0')
        self.assertContains(page, 'row-gap: 0')
        self.assertContains(page, 'border-radius: 0')
    ...
    def test_stampa_cijena_menu_same_and_mixed(self):
        ...
        self.assertContains(zebra, 'size: 2in 1.224in')
        self.assertContains(zebra, 'padding: 0.100in 0.03in 0.03in 0.05in')
        ...
        self.assertContains(zebra, '^PW406')
        ...
        self.assertContains(zebra_mixed, 'size: 2in 1.224in')
    ...
    def test_stampa_cijena_zebra_qr_and_print(self):
        ...
        self.assertContains(zebra, 'label-qr')
        self.assertContains(zebra, '0.52in')
        self.assertContains(zebra, '^BY')
        self.assertContains(zebra, '^BQN,2,3')
        self.assertContains(zebra, 'object-fit: fill')
    ...
    def test_brzi_unos_same_as_admin_flow(self):
        ...
        self.assertContains(act, 'data-image-fit')
        self.assertContains(act, 'name="slika_rucno"')
        self.assertContains(act, 'Povuci sliku')
    ...
    def test_popis_provjera_shows_stock_and_updates(self):
        from .magacin import location_stock_qty

        self.client.force_login(self.user)
        loc = WarehouseLocation.objects.get(sifra='T-1')
        started = self.client.post(reverse('staff_magacin_popis'), {
            'action': 'provjera_start',
            'location_id': loc.pk,
        })
        self.assertEqual(started.status_code, 302)
        self.assertIn('mode=provjera', started['Location'])
        self.assertIn(f'lokacija={loc.pk}', started['Location'])
        looked = self.client.post(
            reverse('staff_magacin_popis'),
            {
                'action': 'provjera_artikal',
                'location_id': loc.pk,
                'product_id': str(self.product.pk),
            },
            HTTP_X_REQUESTED_WITH='XMLHttpRequest',
        ...
        payload = looked.json()
        self.assertTrue(payload['ok'])
        self.assertEqual(payload['na_stanju'], 8)
        self.assertEqual(payload['sifra'], 'TST-1')
        unchanged = self.client.post(
            reverse('staff_magacin_popis'),
            {
                'action': 'provjera_izmijeni',
                'location_id': loc.pk,
                'product_id': str(self.product.pk),
                'kolicina': '8',
            },
        ...
        self.assertEqual(unchanged.json()['na_stanju'], 8)
        self.assertEqual(location_stock_qty(loc, self.product), 8)
        ...
        self.assertEqual(changed.json()['na_stanju'], 5)
        self.assertTrue(changed.json()['izmijenjeno'])
        self.assertEqual(location_stock_qty(loc, self.product), 5)
    ...
    def test_vp_order_persists_customer_and_vp_price(self):
        ...
        self.assertNotContains(listed, reverse('staff_magacin_vp_narudzba'))
        ...
        blocked_pay = self.client.post(reverse('staff_magacin_vp_narudzba'), {'action': 'zavrsi'})
        self.assertEqual(blocked_pay.status_code, 302)
        draft.refresh_from_db()
        self.assertEqual(draft.status, MagacinVpNarudzba.Status.U_TOKU)
        vp_form = self.client.get(reverse('staff_magacin_vp_narudzba'))
        self.assertContains(vp_form, 'id="vpPayModal"')
        self.assertContains(vp_form, 'Žiralno')
        self.assertContains(vp_form, 'Gotovinski')
        finished = self.client.post(reverse('staff_magacin_vp_narudzba'), {
            'action': 'zavrsi',
            'placanje': 'gotovina',
        })
        self.assertEqual(finished.status_code, 302)
        ...
        self.assertEqual(draft.placanje, MagacinVpNarudzba.Placanje.GOTOVINA)
    ...
    def test_vp_ziralno_is_skipped_on_packing_print(self):
        from .views_magacin import apply_order_pick, packing_ready_orders

        self.client.force_login(self.user)

        def make_vp(name, phone, pay):
            self.client.post(reverse('staff_magacin_vp_narudzba'), {'action': 'novi'})
            customer = WarehouseCustomer.objects.create(ime_prezime=name, telefon=phone)
            self.client.post(reverse('staff_magacin_vp_narudzba'), {
                'action': 'kupac', 'customer_id': customer.pk,
            })
            self.client.post(reverse('staff_magacin_vp_narudzba'), {
            ...
            done = self.client.post(reverse('staff_magacin_vp_narudzba'), {
                'action': 'zavrsi', 'placanje': pay,
            })
        ...
        cash = make_vp('VP Gotovina', '061100100', 'gotovina')
        bank = make_vp('VP Ziralno', '061200200', 'ziralno')
        self.assertEqual(cash.packing_placanje_label(), 'GOTOVINSKI')
        self.assertEqual(bank.packing_placanje_label(), 'ŽIRALNO')
        self.assertEqual(MagacinVpNarudzba.objects.get(order=bank).placanje, 'ziralno')
        ready = packing_ready_orders()
        self.assertEqual([order.ime_prezime for order in ready], ['VP Gotovina'])
        listed = self.client.get(reverse('staff_magacin_narudzbe'))
        self.assertEqual(listed.context['packing_ready_count'], 1)
        printed = self.client.get(reverse('staff_magacin_narudzbe_packing'))
        self.assertEqual(printed.status_code, 200)
        self.assertContains(printed, 'VP Gotovina')
        self.assertContains(printed, 'GOTOVINSKI')
        self.assertNotContains(printed, 'VP Ziralno')
        self.assertNotContains(printed, 'ŽIRALNO')
        cash.refresh_from_db()
        bank.refresh_from_db()
        self.assertTrue(cash.packing_odstampana)
        self.assertFalse(bank.packing_odstampana)
    ...
    def test_vp_bulk_imports_matching_names_and_skips_unknown(self):
        ...
        finished = self.client.post(reverse('staff_magacin_vp_narudzba'), {'action': 'zavrsi', 'placanje': 'gotovina'})
        self.assertEqual(finished.status_code, 302)
    ...
    def test_vp_picking_finish_deducts_stock_and_status_is_green(self):
        ...
        self.client.post(reverse('staff_magacin_vp_narudzba'), {'action': 'zavrsi', 'placanje': 'gotovina'})
    ...
    def test_stampa_kolicine_shows_name_sifra_qty(self):
        self.client.force_login(self.user)
        order = Order.objects.create(
            ime_prezime='Faktura Qty',
            telefon='061000111',
        ...
        OrderItem.objects.create(
            ...
            product_naziv=self.product.naziv,
            sifra=self.product.sifra,
            cijena=Decimal('10.00'),
            kolicina=2,
        )
        ...
        self.assertEqual(page.status_code, 200)
        self.assertContains(page, 'Naziv')
        self.assertContains(page, 'Šifra')
        self.assertContains(page, 'Količina')
        self.assertContains(page, self.product.naziv)
        self.assertContains(page, 'TST-1')
        self.assertContains(page, '>2<')
        self.assertNotContains(page, 'KM')
    ...
    def test_narudzbe_has_manual_entry(self):
        ...
        self.assertContains(form, 'id="mgSpareProductSearch"')
        self.assertContains(form, 'Artikal *')
        self.assertContains(form, 'Šta šalješ')
        self.assertContains(form, 'Naplata')
        zalihe = self.client.get(reverse('staff_magacin_zalihe'))
        self.assertContains(zalihe, reverse('staff_magacin_rezervni_dijelovi'))
        self.assertContains(zalihe, 'Rezervni dijelovi')
        ...
        self.assertContains(form, reverse('staff_magacin_narudzba_bulk'))
        self.assertContains(form, 'id="mgOrderBulk"')
        self.assertContains(form, 'id="mgOrderBulkModal"')
        self.assertContains(form, 'Bulk unos')
        self.assertNotContains(form, 'id="mgOrderScan"')
        ...
        self.assertContains(form, 'placeholder="Unesi napomenu"')

    def test_order_list_napomena_strips_system_lines(self):
        from .views_magacin import _order_list_napomena
        self.assertEqual(_order_list_napomena(''), '')
        self.assertEqual(_order_list_napomena('VP narudžba'), '')
        self.assertEqual(
            _order_list_napomena('VP narudžba\nPozvati prije dostave\nPlaćeno karticom'),
            'Pozvati prije dostave',
        )
        self.assertEqual(
            _order_list_napomena('Ostaviti kod susjeda\nNije popisan: Test braid'),
            'Ostaviti kod susjeda',
        )

    def test_manual_order_bulk_resolves_names_without_creating_order(self):
        self.client.force_login(self.user)
        paste = (
            '#,\tArtikal,\tKol.,\tCijena\t,Ukupno\n'
            '1,\tTest braid\t,2\t,8.50 KM\t,17.00 KM\n'
            '2,\tNepostojeci Artikal XYZ\n'
            'Šifra: 9999\t,1\t,10.00 KM\t,10.00 KM\n'
        )
        empty = self.client.post(
            reverse('staff_magacin_narudzba_bulk'),
            {'tekst': '   '},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest',
        )
        ...
        self.assertEqual(data['added'][0]['item']['id'], self.product.pk)
        self.assertEqual(data['added'][0]['qty'], 2)
        self.assertEqual(len(data['skipped']), 1)
        self.assertIn('Nepostojeci Artikal XYZ', data['skipped'][0]['naziv'])
    ...
    def test_picking_completed_shows_older_jobs(self):
        ...
        self.assertContains(done, reverse('staff_magacin_pakuj_detail', args=[old.broj]))
        summary = self.client.get(reverse('staff_magacin_pakuj_detail', args=[old.broj]))
        self.assertEqual(summary.status_code, 200)
        self.assertContains(summary, 'odakle je uzeto')
        self.assertContains(summary, 'Stari Picking')
    ...
    def test_vp_customer_orders_are_vp(self):
        self.client.force_login(self.user)
        saved = self.client.post(reverse('staff_magacin_kupci_save'), {
            'ime_prezime': 'VP Firma',
            'telefon': '061999888',
            'adresa': 'Industrijska 1',
            'grad': 'Sarajevo',
            'postanski_broj': '71000',
            'vp_kupac': '1',
        })
        self.assertEqual(saved.status_code, 200)
        self.assertTrue(saved.json()['customer']['vp_kupac'])
        customer = WarehouseCustomer.objects.get(telefon='061999888')
        self.assertTrue(customer.vp_kupac)
        loc = WarehouseLocation.objects.create(sifra='VP-1', naziv='VP test', redoslijed=1)
        apply_movement(product=self.product, location=loc, tip='prijem', kolicina=5)
        created = self.client.post(reverse('staff_magacin_narudzba_nova'), {
            'ime_prezime': 'VP Firma',
            'telefon': '061999888',
            'adresa': 'Industrijska 1',
    ...
    def test_spare_part_line_on_order_and_picking(self):
        ...
        self.assertEqual(item.artikal_id, self.product.pk)
        self.assertEqual(item.naziv, 'GORNJA SEKCIJA')
        ...
        self.assertIn('GORNJA SEKCIJA', queue[0]['naziv'])
        pick_json = json.dumps([{
            'key': f'{item.pk}:T-1',
            'item_id': item.pk,
            'loc': 'T-1',
        ...
        done_pick = self.client.get(reverse('staff_magacin_pakuj_detail', args=[order.broj]))
        self.assertEqual(done_pick.status_code, 200)
        self.assertContains(done_pick, 'odakle je uzeto')
        self.assertContains(done_pick, 'T-1')
        self.assertContains(done_pick, 'GORNJA SEKCIJA')
        self.assertContains(done_pick, '<td class="num">2</td>')
        mixed = self.client.post(reverse('staff_magacin_narudzba_nova'), {
            'ime_prezime': 'Spare Mix',
            'telefon': '061777889',
            'product_id': [str(self.product.pk), str(self.product.pk)],
            'variation_id': ['', ''],
            'kolicina': ['1', '1'],
            'mp_ok': ['0', '0'],
            'rezervni': ['0', '1'],
        ...
        self.assertEqual(stock_totals(self.product)['rezervisano'], 2)
        listed_spares = self.client.get(reverse('staff_magacin_rezervni_dijelovi'))
        self.assertEqual(listed_spares.status_code, 200)
        self.assertContains(listed_spares, 'class="mg-search-icon"')
        self.assertContains(listed_spares, 'Test braid')
        self.assertContains(listed_spares, 'GORNJA SEKCIJA')
        self.assertContains(listed_spares, 'Donja sekcija')
        self.assertContains(listed_spares, 'mg-spare-missing')
        self.assertContains(listed_spares, f'#{order.broj}')
        found = self.client.get(reverse('staff_magacin_rezervni_dijelovi'), {'pretraga': 'GORNJA'})
        self.assertContains(found, 'GORNJA SEKCIJA')
        self.assertNotContains(found, 'Donja sekcija')
        missing_product = self.client.post(reverse('staff_magacin_narudzba_nova'), {
            'ime_prezime': 'Spare Bez Artikla',
            'telefon': '061777890',
            'product_id': [''],
            'variation_id': [''],
            'kolicina': ['1'],
        ...
        self.assertFalse(Order.objects.filter(ime_prezime='Spare Bez Artikla').exists())
    ...
    def test_manual_order_applies_loyalty_discount_from_phone(self):
        ...
        self.client.force_login(self.user)
        skipped = self.client.post(reverse('staff_magacin_narudzba_nova'), {
        ...
        self.assertFalse(order.kupon_kod)
        self.assertFalse(order.loyalty_popust_info())
        ...
        summary = sazetak_iz_narudzbe(auto_order)
    ...
    def test_popis_test_page_scans_and_writes_counted_qty_to_location(self):
        ...
        self.assertContains(live, 'Otkaži popis')
        self.assertContains(live, 'id="ptCancel"')
        self.assertContains(live, 'pt-finish--danger')
        self.assertNotContains(live, 'Sačuvaj popis')
        self.assertContains(live, 'id="ptQtyInput"')
        self.assertContains(live, 'Popisana količina')
        ...
        self.assertContains(listed, self.product.naziv)
        self.assertContains(listed, 'Ukupno stanje: 8 kom')
        ...
        after_html = after.content.decode()
        self.assertLess(
            after_html.find('Šifra ili naziv lokacije'),
            after_html.find('Završeni popisi'),
        )
    ...
    def test_popis_test_cancel_discards_count_without_writing_stock(self):
        self.client.force_login(self.user)
        loc = WarehouseLocation.objects.get(sifra='T-1')
        self.client.post(
            reverse('staff_magacin_popis_test'),
        ...
        self.assertEqual(scanned.status_code, 200)
        self.assertEqual(scanned.json()['current']['sifra'], 'TST-1')
        cancelled = self.client.post(
            reverse('staff_magacin_popis_test'),
            {'action': 'otkazi'},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest',
        ...
        stock = WarehouseStock.objects.get(product=self.product, location__sifra='T-1')
        self.assertEqual(stock.kolicina, 8)
        self.assertEqual(MagacinPopis.objects.filter(status=MagacinPopis.Status.ZAVRSEN).count(), 0)

    def test_popis_test_zero_stock_starts_counted_qty_at_one(self):
        self.client.force_login(self.user)
        loc = WarehouseLocation.objects.get(sifra='T-1')
        self.client.post(
            reverse('staff_magacin_popis_test'),
        ...
        scanned = self.client.post(
            reverse('staff_magacin_popis_test'),
            {'action': 'scan', 'q': 'ZERO-1'},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest',
        )
        self.assertEqual(scanned.status_code, 200)
        payload = scanned.json()
        self.assertTrue(payload['ok'])
        self.assertEqual(payload['current']['sifra'], 'ZERO-1')
        self.assertEqual(payload['current']['sistem'], 0)
        self.assertEqual(payload['current']['popisano'], 1)
        self.assertEqual(payload['current']['razlika'], 1)
